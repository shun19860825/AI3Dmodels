...
export default function CameraSideScreen() {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.previewActions}>
          ...
          <TouchableOpacity style={styles.confirmBtn} onPress={confirmPhoto}>
            <Text style={styles.confirmBtnText}>解析開始 →</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <CameraGuide mode="side" />

      <View style={styles.stepBadge}>
        <Text style={styles.stepText}>STEP 2 / 2　側面写真（任意）</Text>
      </View>

      <View style={styles.accuracyBadge}>
        <Text style={styles.accuracyText}>
          ✦ 側面写真を撮ると胸囲・ウエスト・ヒップの精度が向上します
        </Text>
      </View>

      <View style={styles.shutterRow}>
        <TouchableOpacity style={styles.skipBtn} onPress={skipSide}>
          <Text style={styles.skipBtnText}>スキップして解析</Text>
        </TouchableOpacity>
        <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  accuracyBadge: {
    ...
    backgroundColor: 'rgba(0,212,170,0.15)', borderRadius: 8,
    padding: 10,
  },
  accuracyText: { color: '#00d4aa', fontSize: 12, textAlign: 'center' },
  shutterRow: {
  ...
  skipBtnText: { color: '#aaa', fontWeight: '700', fontSize: 13 },
  shutter: {
  ...
  preview: { flex: 1 },
  previewActions: {
    flexDirection: 'row', backgroundColor: '#0f0f1a', padding: 16, gap: 12,
  },
  retakeBtn: {
